# Константы путей (измените под ваши пути на Google Drive)
DATA_DIR = "plant_diseases/"  # Путь к папке с датасетом
MODEL_SAVE_PATH = "models/convnext_arcface"  # Путь для сохранения модели
...
class Config:
    ...
    LR = 0.0026
    NUM_CLASSES = 37
    IMG_SIZE = 128  # Для экономии памяти
...
class DataProcessor:
    ...
    def get_loaders(self):
        ...
        sample_image, _ = next(iter(datasets.ImageFolder(DATA_DIR, transform=transforms.ToTensor())))
        full_dataset = datasets.ImageFolder(DATA_DIR, transform=self.vall_transform)
...
class ArcFace(nn.Module):
    def __init__(self, in_features, out_features, scale=32.0, margin=0.5):
        ...
        print(f"Initial W norm before normalize: {torch.norm(self.W)}")
        # Маска для выборки положительных классов
        self.one_hot = None
        self.cos_m = math.cos(margin)
        self.sin_m = math.sin(margin)
        self.th = math.cos(math.pi - margin)
        self.mm = math.sin(math.pi - margin) * margin
        norms = torch.norm(self.W, p=2, dim=1)
        print(f"Norms per row: {norms}")
        print(f"Total W norm: {torch.norm(self.W)}")
    ...
    def forward(self, x, labels=None):
        ...
        # Применение маржи только к положительным классам
...
class DiseaseClassifier(nn.Module):
    def __init__(self, num_classes, feature_dim=1280, head_type='arcface'):
        ...
        if head_type == 'arcface':
            self.head = ArcFace(768, num_classes, scale=64.0, margin=0.5)
        else:
            raise ValueError("Only 'arcface' head_type is supported in this version.")
    ...
    def run_epoch(self, loader, is_train=True):
        ...
        for batch_idx, (inputs, labels) in enumerate(pbar):
            ...
            if is_train:
                ...
                self.scaler.update()
                print(f"ArcFace W norm after step: {torch.norm(self.model.head.W)}")
        ...
        # print(f"Batch {batch_idx}: Correct predictions: {correct}/{total}, Acc={acc:.2f}%")
...
if __name__ == "__main__":
    ...
    class_counts = torch.zeros(Config.NUM_CLASSES)
    for _, label in datasets.ImageFolder(DATA_DIR, transform=processor.vall_transform):
    ...
    # print("Sample labels:", next(iter(train_loader))[1])
    ...
    # lr_finder.plot()  # Сохранит график
    # lr_finder.reset()  # Вернёт веса
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            counter = 0
            torch.save(model.state_dict(), MODEL_SAVE_PATH+ f"_{epoch}" + f"_{best_acc * 100 // 1}" + ".pth")
