...
METRICS_DIR = "metrics/"
MISCLASSIFIED_TXT = "metrics/Нужные удаления/misclassified_images.txt"  # Укажи путь к твоему txt файлу
...
class Config:
    ...
    LR = 0.0026  # Уменьшен для плавного обучения
    NUM_CLASSES = 35
    IMG_SIZE = 128
    ...
    EMBEDDING_DIM = 1280
    MARGIN = 1.0
    TRAIN_RATIO = 0.8  # 80% для тренировки, 20% для валидации
    DROPOUT = 0.2 
    WEIGHT_DECAY = 1e-4
    GRAD_CLIP_NORM = 10.0
...
class DataProcessor:
    ...
    def clean_dataset(self):
        if not os.path.exists(MISCLASSIFIED_TXT):
            print(f"File {MISCLASSIFIED_TXT} not found. Skipping cleaning.")
            return
    ...
    def get_loaders(self):
        self.clean_dataset()  # Удаляем файлы перед загрузкой
        full_dataset = datasets.ImageFolder(DATA_DIR, transform=self.val_transform)
        classes, class_to_idx = full_dataset.find_classes(DATA_DIR)
        ...
        train_labels = [full_dataset.imgs[i][1] for i in train_dataset.indices]
...
def plot_confusion_matrix(model, val_loader, metrics_dir):
    model.eval()
    all_preds = []
    all_labels = []
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            all_preds.extend(preds.cpu().numpy())
            all_labels.extend(labels.cpu().numpy())
    class_names = ['Alternaria leaf blight', 'Anthocyanosis', 'Anthracnose', 'Aphid', 'Aphid effects', 'Ascochyta blight', 'Bacterial spot', 'Black chaff', 'Black rot', 'Black spots', 'Botrytis cinerea', 'Burn', 'Downy mildew', 'Dry rot', 'Edema', 'Grey mold', 'Healthy', 'Late blight', 'Leaf deformation', 'Leaf miners', 'Loss of foliage turgor', 'Marginal leaf necrosis', 'Mealybug', 'Mechanical damage', 'Mosaic virus', 'Nutrient deficiency', 'Powdery mildew', 'Rust', 'Scale', 'Shot hole', 'Sooty mold', 'Spider mite', 'Thrips', 'Whitefly', 'Wilting']
    cm = confusion_matrix(all_labels, all_preds, normalize='true')
    plt.figure(figsize=(20, 20))
    sns.heatmap(cm, annot=True, fmt='.2f', cmap='Blues', xticklabels=class_names[:Config.NUM_CLASSES], yticklabels=class_names[:Config.NUM_CLASSES], annot_kws={"size": 10})
...
if __name__ == "__main__":
    ...
    train_loader, val_loader = processor.get_loaders()  # Разделение после очистки
    class_counts = torch.zeros(Config.NUM_CLASSES)
    for _, label in datasets.ImageFolder(DATA_DIR, transform=processor.val_transform):
        class_counts[label] += 1
    print(f"Class distribution: {class_counts}")
    show_batch(train_loader)
    model = DiseaseClassifier(num_classes=Config.NUM_CLASSES, stage='stage1')  # Новая модель
    ...
    plot_confusion_matrix(model, val_loader, metrics_dir)