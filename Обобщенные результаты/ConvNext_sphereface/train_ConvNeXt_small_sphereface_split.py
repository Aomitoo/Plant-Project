...
MODEL_SAVE_PATH = "models/convnext_small_sphereface"
METRICS_DIR = "metrics/"
...
class Config:
    ...
    BACKBONE_FEATURE_DIM = 768  # Updated for ConvNeXt_small
    EMBEDDING_DIM = 1280
    MARGIN = 2.0
    TRAIN_RATIO = 0.8
    DROPOUT = 0.2 
...
class DataProcessor:
    def __init__(self):
        self.mean = [0.4569, 0.5046, 0.3590]
        self.std = [0.2169, 0.2138, 0.2120]
    ...
    def get_loaders(self):
        """Load data from pre-split train and val folders."""
        train_dataset = datasets.ImageFolder(os.path.join(DATA_DIR, 'train'), transform=self.train_transform)
        val_dataset = datasets.ImageFolder(os.path.join(DATA_DIR, 'val'), transform=self.val_transform)
        
        classes, class_to_idx = train_dataset.find_classes(os.path.join(DATA_DIR, 'train'))
        ...
        weights = torch.tensor([1.0 / class_counts[label] for _, label in train_dataset])
        sampler = WeightedRandomSampler(weights, len(weights))
        
        return (
            DataLoader(train_dataset, batch_size=Config.BATCH_SIZE, sampler=sampler, num_workers=2, pin_memory=True),
...
class SphereFace(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            print(f"Cos theta min: {cos_theta.min()}, max: {cos_theta.max()}")  # Отладка
        if labels is None:
...
class DiseaseClassifier(nn.Module):
    def __init__(self, num_classes, stage='stage1'):
        super().__init__()
        self.backbone = models.convnext_small(weights='IMAGENET1K_V1')  # Updated to ConvNeXt_small
        self.backbone.classifier = nn.Flatten(start_dim=1)  # Flatten to match embedding layer input
        self.embedding = nn.Linear(Config.BACKBONE_FEATURE_DIM, Config.EMBEDDING_DIM)
        self.dropout = nn.Dropout(Config.DROPOUT)
        self.stage = stage
        if stage == 'stage1':
            self.head = SphereFace(Config.EMBEDDING_DIM, num_classes, m=Config.MARGIN)
        else:
...
class Trainer:
    ...
    def log_misclassified_images(self, loader, metrics_dir, class_names, stage, epoch):
        """Log misclassified images during validation to a text file."""
        self.model.eval()
        misclassified = []
        ...
        with torch.no_grad():
            for batch_idx, (inputs, labels) in enumerate(tqdm(loader, desc=f"Logging misclassified (Stage {stage}, Epoch {epoch})")):
        ...
        with open(f"{metrics_dir}/misclassified_images_stage{stage}_epoch{epoch}.txt", "w") as f:
        ...
        print(f"Misclassified images logged to {metrics_dir}/misclassified_images_stage{stage}_epoch{epoch}.txt")
...
def plot_metrics(trainer, metrics_dir, stage):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"{metrics_dir}/training_metrics_stage{stage}.png")
    plt.close()
...
def save_metrics_to_csv(trainer, metrics_dir, stage):
    ...
    metrics_df.to_csv(f"{metrics_dir}/epoch_metrics_stage{stage}.csv", index=False)

def plot_confusion_matrix(model, test_loader, metrics_dir, class_names, stage):
    model.eval()
    all_preds = []
    all_labels = []
    with torch.no_grad():
    ...
    sns.heatmap(cm, annot=True, fmt='.2f', cmap='Blues', xticklabels=class_names[:Config.NUM_CLASSES], yticklabels=class_names[:Config.NUM_CLASSES])
    ...
    plt.tight_layout()
    plt.savefig(f"{metrics_dir}/confusion_matrix_stage{stage}.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    class_counts = torch.zeros(Config.NUM_CLASSES)
    for _, label in datasets.ImageFolder(os.path.join(DATA_DIR, 'train'), transform=processor.val_transform):
        class_counts[label] += 1
    print(f"Class distribution: {class_counts}")
    class_names = train_loader.dataset.classes  # Get class names for misclassified logging
    ...
    for epoch in range(Config.NUM_EPOCHS_STAGE1):
        ...
        save_metrics_to_csv(trainer, metrics_dir, 1)
        plot_metrics(trainer, metrics_dir, 1)
        # Log misclassified images on epoch 30
        if epoch + 1 == Config.NUM_EPOCHS_STAGE1:
            trainer.log_misclassified_images(val_loader, metrics_dir, class_names, stage=1, epoch=epoch+1)
            plot_confusion_matrix(model, val_loader, metrics_dir, class_names, stage=1)
    ...
    for epoch in range(Config.NUM_EPOCHS_STAGE2):
        ...
        save_metrics_to_csv(trainer, metrics_dir, 2)
        plot_metrics(trainer, metrics_dir, 2)
    ...
    plot_confusion_matrix(model, test_loader, metrics_dir, class_names, stage=2)
    metrics_df = pd.DataFrame({
        'epoch': [len(trainer.val_losses)],
        'train_loss': [sum(trainer.all_train_losses[-trainer.num_train_batches_per_epoch:]) / trainer.num_train_batches_per_epoch],
        'val_loss': [trainer.val_losses[-1]],
        'val_acc': [trainer.val_accs[-1]],
        'val_precision': [trainer.val_precisions[-1]],
        'val_recall': [trainer.val_recalls[-1]],
        'val_f1': [trainer.val_f1s[-1]],
        'test_loss': [test_loss],
        'test_acc': [test_acc],
    ...
    metrics_df.to_csv(f"{metrics_dir}/final_metrics.csv", index=False)