...
transform = A.Compose([
    A.SmallestMaxSize(max_size=128, interpolation=1),
...
for class_name in os.listdir(DATA_DIR):
    ...
    if os.path.isdir(class_dir):
        ...
        augment_count = 5 if num_images < 50 else 2
        target_count = min(2, num_images * augment_count)  # Ограничение до 200 изображений
        current_aug_count = 0
        for img_name in os.listdir(class_dir):
            ...
            for i in range(augment_count):
                ...
                augmented_image.save(os.path.join(aug_class_dir, f"aug_{i}_1_{img_name}"))
                current_aug_count += 1
