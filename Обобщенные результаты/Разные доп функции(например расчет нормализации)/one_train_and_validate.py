...
class Config:
    ...
    MODEL_PATH = "models/BEST_efficientnet_sphereface_ 85.25% copy.pth"
    NUM_EPOCHS = 5  # Set your desired number of epochs here
...
class SphereFace(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            print(f"Cos theta min: {cos_theta.min():.4f}, max: {cos_theta.max():.4f}")
        if labels is None:
...
def train_and_validate():
    ...
    for epoch in range(Config.NUM_EPOCHS):
        ...
        for batch_idx, (inputs, labels) in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch+1}/{Config.NUM_EPOCHS}")):
            try:
                ...
                iter_loss = loss.item()
                iter_accuracy = (preds == labels).float().mean().item() * 100
        ...
        print(f"Epoch {epoch+1}/{Config.NUM_EPOCHS} | Train Loss: {train_loss:.4f} | Acc: {train_acc:.2f}%")
        ...
        all_labels = []
        misclassified = []
        image_paths = [sample[0] for sample in val_dataset.samples]

        with torch.no_grad():
            for batch_idx, (inputs, labels) in enumerate(tqdm(val_loader, desc=f"Validating Epoch {epoch+1}/{Config.NUM_EPOCHS}")):
                try:
                    inputs = inputs.to(Config.DEVICE)
                    labels = labels.to(Config.DEVICE)
        ...
        print(f"Epoch {epoch+1}/{Config.NUM_EPOCHS} | Val Loss: {val_loss:.4f} | Acc: {val_acc:.2f}%")

        # Generate confusion matrix and save metrics for the last epoch
